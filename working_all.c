...
void insert_blk(void * pp) {
	...
	PUTP(PRED(pp), NULL);
	PUTP(PRED(listp), pp); // why?
	listp = pp;
}

void remove_blk(void * pp) {
	...
	if (prev == NULL) {
		// If the block is the first one in the list
		listp = next;	
	}
	else {
		PUTP(SUCC(prev), next);
...
//	printf("ptr sizeof %d\n", sizeof(void *));
	...
	heapp = mem_sbrk(2*MBLK_SIZE /*4 * WSIZE*/);
	if (heapp == (void*)-1) {
		return -1;
	}
	...
	PUT(heapp, 0);
	PUT((char *)heapp + WSIZE, PACK(MBLK_SIZE, 1));
	PUT((char *)heapp + MBLK_SIZE, PACK(MBLK_SIZE, 1));
	PUT((char *)heapp + MBLK_SIZE + WSIZE, PACK(0, 1));
	heapp =(char *)heapp +  2 * WSIZE;


	PUT((char *)heapp + DSIZE, 0);
	PUT((char *)heapp + DSIZE + WSIZE, 0);
